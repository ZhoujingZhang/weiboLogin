...
public class SinaWeibo
{
	...
	/*the first way for weibo login
	 * 参考网页http://www.cnblogs.com/longjin-java/p/6408360.html
	 * @author zhangzhoujing
	 * */
	public boolean preLogin() {
		boolean flag = false;
		try {
			su = new String(Base64.getEncoder().encode(URLEncoder.encode(username, "UTF-8").getBytes()));
			String urlpath = "https://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su="+su+
				 "&rsakt=mod&checkpin=1&Client=ssologin.js(v1.4.19)&_="+getTimestamp();
			String content ;
			content = this.httpUtil.getRequest(client,urlpath);
			String regex = "sinaSSOController.preloginCallBack\\(([\\s\\S]*?)\\)";
			Pattern p  = Pattern.compile(regex);
			Matcher m = p.matcher(content);
			if(m.find()) {
				String prelocation = m.group(1);
				JSONObject json = JSONObject.fromObject(prelocation);
	...
	private static String getTimestamp() {
		...
		timestamp = nousedate.getTime();
		timestamp = timestamp/1000;
		System.out.println("time="+timestamp);
		return String.valueOf(timestamp);
	}

	public boolean login_demo()
	{
		List <NameValuePair> parms = new ArrayList<NameValuePair>();
		String urlLogin = "http://login.sina.com.cn/sso/login.php?client=ssologin.js(v1.4.15)";
		if(preLogin())
		{
			...
			parms.add(new BasicNameValuePair("qrcode_flag", "false"));
	        ...
	        parms.add(new BasicNameValuePair("sr", "1536*864"));
	        parms.add(new BasicNameValuePair("encoding", "UTF-8"));
	        parms.add(new BasicNameValuePair("prelt", "129"));
	        parms.add(new BasicNameValuePair("url", "https://weibo.com/ajaxlogin.php?framelogin=1&callback=parent.sinaSSOController.feedBackUrlCallBack")); 
	        parms.add(new BasicNameValuePair("returntype", "META"));
		}
		try {
			String content =  this.httpUtil.postRequest(client,urlLogin,parms);
			
			String regex_new = "([\\s\\S]*?)location.replace\\(\'([\\s\\S]*?)\'\\);}\\);";
			Pattern p  = Pattern.compile(regex_new);
			Matcher m = p.matcher(content);
			if(m.find())
			{
				location = m.group(2);
				if(location.contains("reason=")) {//如果你走进了这一步，恭喜报错了
                    ...
                    errInfo = URLDecoder.decode(errInfo, "GBK");
                     ...
                     String result = this.httpUtil.getRequest(client, location);//.substring(2, location.length()-2)
                   ...
                   System.out.println("result--------------" + result+"uniqueid:"+uniqueid + "userdomain:"+userdomain);
                   return true;
                 }
			}
        }  catch (IOException e) {
	...
	/*the second way 
	 * 参考网页 http://www.fullstackyang.com/archives/434.html
	 * @author zhangzhoujing
	 */
